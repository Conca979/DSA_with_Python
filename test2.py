...
class BSTree:
  ...
  def isBalanced(self, node=None):
    ...
    if node is None:
      return _check_(node=self.root)
    else:
      return _check_(node=node)

  def balance(self):
      # print(f"right rotate on {node.val}","-"*40)
    ...
    def balance_factor(node):
      return self.level(node.left) - self.level(self.right)
    ...
    def _run(node=self.root):
      ...
      # start balancing
      b_factor = balance_factor(node)
      
      # left rotation
      for _ in range(-balance_factor//2):
        pass
